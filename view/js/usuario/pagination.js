 ...
 * @returns {void}
 */
function fetchUsuarios(page, size, sort) {
    // Realizar solicitud GET al servidor para obtener la lista de usuarios
    fetch(`../controller/usuarioAction.php?page=${page}&size=${size}&sort=${sort}`)
        .then(response => response.json())
        .then(data => {
            // Verificar si la respuesta fue exitosa
            if (data.success) {
                // Renderizar la tabla de usuarios con los datos obtenidos
...
function changePage(newPage) {
    ...
    if (newPage >= 1 && newPage <= totalPages) {
        ...
        fetchUsuarios(newPage, pageSize, sort);
    }
}
