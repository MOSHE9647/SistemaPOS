 ...
 * @returns {undefined}
 */
function fetchCuentaPorPagar(page, size) {
    fetch(`../controller/CuentaPorPagarAction.php?page=${page}&size=${size}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                console.log(data); // Verifica que data contiene la información esperada
                renderTable(data.listaCuentaPorPagar);
        ...
        .catch(error => {
            ...
            showMessage(`Ocurrió un error al obtener la lista de cuenta por pagar.<br>${error}`, 'error');
        });
}
