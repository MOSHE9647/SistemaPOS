 ...
 * 
 * @description Esta función realiza una solicitud GET al servidor para obtener la lista de categorias,
 *              procesa la respuesta y actualiza la tabla de categorias en la página.
 * 
 * @param {number} page - El número de página a obtener
 * @param {number} size - El tamaño de la página (número de registros por página)
 * @param {string} sort - El campo por el que ordenar la lista de categorias
 * 
 * @example
 * fetchCategorias(1, 10, 'tipo');
 * 
 * @returns {void}
 */
function fetchCategorias(page, size, sort) {
    // Realizar solicitud GET al servidor para obtener la lista de categorias
    fetch(`../controller/categoriaAction.php?page=${page}&size=${size}&sort=${sort}`)
        .then(response => response.json())
        .then(data => {
            // Verificar si la respuesta fue exitosa
            if (data.success) {
                // Renderizar la tabla de categorias con los datos obtenidos
                ...
                // Actualizar contcategorias de paginación
...
/**
 * Actualiza los contcategorias de paginación en la página.
 * 
...
/**
 * Cambia la página actual en la lista de categorias.
 * 
 * @description Esta función verifica si la nueva página es válida (entre 1 y el total de páginas)
 *              y, si es así, llama a la función fetchCategorias para obtener la lista de categorias
...
function changePage(newPage) {
    ...
    if (newPage >= 1 && newPage <= totalPages) {
        // Llamar a la función fetchCategorias para obtener la lista de categorias de la nueva página
        fetchCategorias(newPage, pageSize, sort);
    }
}

/**
 * Cambia el tamaño de página actual en la lista de categorias.
 * 
 * @description Esta función actualiza el tamaño de página y llama a la función fetchCategorias
 *              para obtener la lista de categorias correspondiente a la página actual con el nuevo tamaño.
 * 
...
function changePageSize(newSize) {
    ...
    pageSize = newSize;
    // Llamar a la función fetchCategorias para obtener la lista de categorias con el nuevo tamaño
    fetchCategorias(currentPage, pageSize, sort);
}

/**
 * Cambia el orden de la lista de categorias.
 * 
 * @description Esta función actualiza el orden de la lista y llama a la función fetchCategorias
 *              para obtener la lista de categorias correspondiente a la página actual con el nuevo orden.
 * 
...
function changePageSort(newSort) {
    ...
    sort = newSort;
    // Llamar a la función fetchCategorias para obtener la lista de categorias con el nuevo orden
    fetchCategorias(currentPage, pageSize, sort);
}
