...
/**
 * Obtiene la lista de subcategorias desde el servidor.
 * 
 * @description Esta función realiza una solicitud GET al servidor para obtener la lista de subcategorias,
 *              procesa la respuesta y actualiza la tabla de subcategorias en la página.
 * 
 * @param {number} page - El número de página a obtener
 * @param {number} size - El tamaño de la página (número de registros por página)
 * @param {string} sort - El campo por el que ordenar la lista de subcategorias
 * 
 * @example
 * fetchSubcategorias(1, 10, 'tipo');
 * 
 * @returns {void}
 */
function fetchSubcategorias(productoID, page, size, sort) {
    // Realizar solicitud GET al servidor para obtener la lista de subcategorias
    fetch(`../controller/productoSubcategoriaAction.php?page=${page}&size=${size}&sort=${sort}&producto=${productoID}`)
        .then(response => response.json())
        .then(data => {
            // Verificar si la respuesta fue exitosa
            if (data.success) {
                // Renderizar la tabla de subcategorias con los datos obtenidos
                renderTable(data.subcategorias);
...
/**
 * Cambia la página actual en la lista de subcategorias.
 * 
 * @description Esta función verifica si la nueva página es válida (entre 1 y el total de páginas)
 *              y, si es así, llama a la función fetchSubcategorias para obtener la lista de subcategorias
...
function changePage(newPage) {
    ...
    if (newPage >= 1 && newPage <= totalPages) {
        // Llamar a la función fetchSubcategorias para obtener la lista de subcategorias de la nueva página
        fetchSubcategorias(producto, newPage, pageSize, sort);
    }
}

/**
 * Cambia el tamaño de página actual en la lista de subcategorias.
 * 
 * @description Esta función actualiza el tamaño de página y llama a la función fetchSubcategorias
 *              para obtener la lista de subcategorias correspondiente a la página actual con el nuevo tamaño.
 * 
...
function changePageSize(newSize) {
    ...
    pageSize = newSize;
    // Llamar a la función fetchSubcategorias para obtener la lista de subcategorias con el nuevo tamaño
    fetchSubcategorias(producto, currentPage, pageSize, sort);
}

/**
 * Cambia el orden de la lista de subcategorias.
 * 
 * @description Esta función actualiza el orden de la lista y llama a la función fetchSubcategorias
 *              para obtener la lista de subcategorias correspondiente a la página actual con el nuevo orden.
 * 
...
function changePageSort(newSort) {
    ...
    sort = newSort;
    // Llamar a la función fetchSubcategorias para obtener la lista de subcategorias con el nuevo orden
    fetchSubcategorias(producto, currentPage, pageSize, sort);
}
