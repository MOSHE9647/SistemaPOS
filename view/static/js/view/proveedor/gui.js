...
import { updateProveedor } from "./crud.js";
    ...
    proveedores.forEach(proveedor => {
        ...
        let row = `
            ...
            <td data-field="categoria">${proveedor.Categoria.Nombre}</td>
                ...
                ` : ''}
            <td class="actions">
                <button class="btn-info las la-info" onclick="gui.infoProveedor(${proveedor.ID})"></button>
 ...
 * infoProveedor(123);
 */
export async function infoProveedor(proveedorID) {
 ...
 * @returns {void}
 */
export async function editProveedor(proveedorID) {
    const proveedor = proveedores.find(proveedor => proveedor.ID == proveedorID);
    if (!proveedor) {
        mostrarMensaje('No se encontró el proveedor solicitado.', 'error', 'Proveedor no encontrado');
        return;
